    ...
    , m_filter(&filter)
{
    ...
    // 初始化接收数据缓冲区
    m_receiveData.read().data = new uint8_t[64];
    m_receiveData.swap();
    m_receiveData.read().data = new uint8_t[64];
}
FDCAN &FDCAN::init()
...
EsfReturnType FDCAN::_rxCallback()
{
    ...
    m_receiveData.swap(); // 切换缓冲区，准备接收下一条消息
    // 将接收到的消息写入接收队列
    m_receiveErrCode = this->write_queue.pushFromISR(m_receiveData.read());
    // 调用自定义接收回调函数
    auto errcode = this->m_rxCallbackFunc(m_receiveData.read());
    return errcode;
}
